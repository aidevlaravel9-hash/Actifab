...
class PanelAccessController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = $query->orderBy('panel_access_id', 'desc')->get();
    ...
    {
        $PanelAccess = PanelAccess::findOrFail($id);
        $PanelAccess->delete();

        return redirect()
            ->route('panel-access.index')
            ->with('success', 'Panel Access deleted successfully.');
    }
