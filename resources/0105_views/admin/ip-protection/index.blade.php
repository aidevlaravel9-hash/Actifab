        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header">Add IP Protection</div>
                            <div class="card-body">
                                <form action="{{ route('ip-protection.store') }}" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label>IP Protection <span style="color:red;">*</span></label>
                                        <input type="text" name="ip_protection" class="form-control"
                                            value="{{ old('ip_protection') }}">
                                        @if ($errors->has('ip_protection'))
                                            <span class="text-danger">{{ $errors->first('ip_protection') }}</span>
                                        @endif
                                    </div>
                                    <div class="mb-3">
                                        <label>Status</label>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                {{--  <form method="GET" class="d-flex">
                                    <input type="text" name="search" class="form-control me-2"
                                        value="{{ request('search') }}" placeholder="Search IP">
                                    <button class="btn btn-outline-primary me-2">Search</button>
                                    <a href="{{ route('ip-protection.index') }}" class=" btn btn-outline-secondary">
                                        Clear
                                    </a>
                                </form>  --}}
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="ipProtection" class="table table-hover">
                                        <thead>
                                            <tr>
                                            ...
                                            @foreach ($data as $row)
                                                <tr class="text-center">
                                                    <td><input type="checkbox" class="recordCheckbox"
                                                            value="{{ $row->ip_protection_id }}"></td>
                                                    <td>
                                                        <button class="btn btn-sm btn-info editBtn"
                                                            data-id="{{ $row->ip_protection_id }}"
                                                            data-value="{{ $row->ip_protection }}"
                ...
                <div class="modal fade" id="editModal" tabindex="-1">
                    <div class="modal-dialog">
                        <form method="POST" action="{{ route('ip-protection.update') }}" class="modal-content">
                            @csrf
                            <input type="hidden" name="edit_id" id="edit_id">
                            <div class="modal-header">
                                <h5 class="modal-title">Edit IP Protection</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label>IP Protection <span style="color:red;">*</span></label>
                                    <input type="text" name="ip_protection" id="edit_value" class="form-control">
                                </div>
                                <div class="mb-3">
                                    <label>Status</label>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#ipProtection').DataTable({
                pageLength: 25,
                lengthMenu: [25, 50, 100 , 125],
                language: {
                    search: "",
                    searchPlaceholder: "Search IP Protection..."
                },
    ...
    <script>
        $(document).on('click', '.toggleStatus', function() {
            ...
            $.post("{{ route('ip-protection.status') }}", {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(res) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                var url = "{{ route('ip-protection.delete', ':id') }}";
        ...
        $('#deleteSelected').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('ip-protection.bulkDelete') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
