...
class GlandPlateThicknessController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = $query->orderByDesc('gland_plate_thickness_id')->get();
    ...
    {
        $GlandPlateThickness = GlandPlateThickness::findOrFail($id);
        $GlandPlateThickness->delete();

        return redirect()
            ->route('gland-plate-thickness.index')
            ->with('success', 'Gland Plate Thickness deleted successfully.');
    }
