...
dd($project); 
    ...
    // Get current financial year (e.g., 2025-26 → 2526)
    $year = date('y'); // 25
    $nextYear = date('y', strtotime('+1 year')); // 26
    $financialYear = $year . $nextYear; // 2526

    // Count existing panels for this project
    $count = ProjectPanelBoard::where('project_id', $project_id)->count();

    // Increment count
    $nextNumber = $count + 1;

    // Final format: 2526-005-3-1
    $jobNo = $financialYear . '-' . $project->inquiry_no . '-3-' . $nextNumber;
