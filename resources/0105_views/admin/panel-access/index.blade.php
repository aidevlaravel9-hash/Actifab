...
@section('content')
        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header">Add Panel Access</div>
                            <div class="card-body">
                                <form action="{{ route('panel-access.store') }}" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label>Panel Access <span style="color:red;">*</span></label>
                                        <input type="text" name="panel_access" class="form-control"
                                            value="{{ old('panel_access') }}">
                                        @if ($errors->has('panel_access'))
                                            <span class="text-danger">{{ $errors->first('panel_access') }}</span>
                                        @endif
                                    </div>
                                    <div class="mb-3">
                                        <label>Status</label>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                {{--  <form method="GET" class="d-flex">
                                    <input type="text" name="search" class="form-control me-2"
                                        value="{{ request('search') }}" placeholder="Search Panel Access">
                                    <button class="btn btn-outline-primary me-2">Search</button>
                                    <a href="{{ route('panel-access.index') }}" class=" btn btn-outline-secondary">
                                        Clear
                                    </a>
                                </form>  --}}
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="panelaccess" class="table table-hover">
                                        <thead>
                                            <tr>
                                            ...
                                            @foreach ($data as $row)
                                                <tr class="text-center">
                                                    <td><input type="checkbox" class="recordCheckbox"
                                                            value="{{ $row->panel_access_id }}"></td>

                                                    <td>
                                                        <button class="btn btn-sm btn-info editBtn"
                                                            data-id="{{ $row->panel_access_id }}"
                                                            data-value="{{ $row->panel_access }}"
    ...
    <div class="modal fade" id="editModal" tabindex="-1">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('panel-access.update') }}" class="modal-content">
                @csrf
                <input type="hidden" name="edit_id" id="edit_id">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Panel Access</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label>Panel Access <span style="color:red;">*</span></label>
                        <input type="text" name="panel_access" id="edit_value" class="form-control">
                    </div>
                    <div class="mb-3">
                        <label>Status</label>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#panelaccess').DataTable({
                pageLength: 25,
                lengthMenu: [ 25, 50, 100 , 125],
                language: {
                    search: "",
                    searchPlaceholder: "Search Panel Access..."
                },
    ...
    <script>
        $(document).on('click', '.toggleStatus', function() {
            ...
            $.post("{{ route('panel-access.status') }}", {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(res) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                var url = "{{ route('panel-access.delete', ':id') }}";
        ...
        $('#deleteSelected').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('panel-access.bulkDelete') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
