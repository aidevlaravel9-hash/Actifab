...
@section('title', 'No of Poles')

@section('content')
<style>
#sectiontype tbody tr:hover {
        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header">Add No of Poles</div>
                            <div class="card-body">
                                <form action="{{ route('poles.store') }}" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label>No of Poles <span style="color:red;">*</span></label>
                                        <input type="number" name="no_of_poles" class="form-control"
                                            value="{{ old('no_of_poles') }}">
                                        @if ($errors->has('no_of_poles'))
                                            <span class="text-danger">{{ $errors->first('no_of_poles') }}</span>
                                        @endif
                                    </div>
                                    <div class="mb-3">
                                        <label>Status</label>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                {{--  <form method="GET" class="d-flex">
                                    <input type="text" name="search" class="form-control me-2"
                                        value="{{ request('search') }}" placeholder="Search No of Poles">
                                    <button class="btn btn-outline-primary me-2">Search</button>
                                    <a href="{{ route('poles.index') }}" class=" btn btn-outline-secondary">
                                        Clear
                                    </a>
                                </form>  --}}
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="poles" class="table table-hover">
                                        <thead>
                                            <tr>
                                            ...
                                            @foreach ($data as $row)
                                                <tr class="text-center">
                                                    <td><input type="checkbox" class="recordCheckbox"
                                                            value="{{ $row->no_of_poles_id }}"></td>
                                                    <td>
                                                        <div class="d-flex justify-content-center gap-2">
                                                            <button class="btn btn-sm btn-info editBtn"
                                                                data-id="{{ $row->no_of_poles_id }}"
                                                                data-value="{{ $row->no_of_poles }}"
                ...
                <div class="modal fade" id="editModal" tabindex="-1">
                    <div class="modal-dialog">
                        <form method="POST" action="{{ route('poles.update') }}" class="modal-content">
                            @csrf
                            <input type="hidden" name="edit_id" id="edit_id">
                            <div class="modal-header">
                                <h5 class="modal-title">Edit No of Poles</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label>No of Poles <span style="color:red;">*</span></label>
                                    <input type="number" name="no_of_poles" id="edit_value" class="form-control">
                                </div>
                                <div class="mb-3">
                                    <label>Status</label>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#poles').DataTable({
                ...
                language: {
                    search: "",
                    searchPlaceholder: "Search No of Poles..."
                },
    ...
    <script>
        $('#deleteRecordModal').on('show.bs.modal', function(event) {
            ...
            $('#rating-delete-form').attr(
                'action',
                '/admin/poles/delete/' + id
            );
