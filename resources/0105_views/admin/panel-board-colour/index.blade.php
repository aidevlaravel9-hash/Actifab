...
@section('title', 'Panel Board Colour')
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('panel-board-colour.store') }}" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label>Colour <span style="color:red;">*</span></label>
                                        <input type="text" name="panel_board_colour" class="form-control"
                                            value="{{ old('panel_board_colour') }}">
                                        @if ($errors->has('panel_board_colour'))
                                            <span class="text-danger">{{ $errors->first('panel_board_colour') }}</span>
                                        @endif
                                    </div>
                                    <div class="mb-3">
                                        <label>Status</label>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                {{--  <form method="GET" class="d-flex">
                                    <input type="text" name="search" class="form-control me-2"
                                        value="{{ request('search') }}" placeholder="Search Colour">
                                    <button class="btn btn-outline-primary me-2">Search</button>
                                    <a href="{{ route('panel-board-colour.index') }}" class=" btn btn-outline-secondary">
                                        Clear
                                    </a>
                                </form>  --}}
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="panelcolor" class="table table-hover">
                                        <thead>
                                            <tr>
                                            ...
                                            @foreach ($data as $row)
                                                <tr class="text-center">
                                                    <td><input type="checkbox" class="recordCheckbox"
                                                            value="{{ $row->panel_board_colour_id }}"></td>
                                                    <td>
                                                        <button class="btn btn-sm btn-info editBtn"
                                                            data-id="{{ $row->panel_board_colour_id }}"
                                                            data-value="{{ $row->panel_board_colour }}"
                ...
                <div class="modal fade" id="editModal" tabindex="-1">
                    <div class="modal-dialog">
                        <form method="POST" action="{{ route('panel-board-colour.update') }}" class="modal-content">
                            @csrf
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label>Colour <span style="color:red;">*</span></label>
                                    <input type="text" name="panel_board_colour" id="edit_value" class="form-control">
                                </div>
                                <div class="mb-3">
                                    <label>Status</label>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#panelcolor').DataTable({
                pageLength: 25,
                lengthMenu: [25, 50, 100 , 125],
                language: {
                    search: "",
                    searchPlaceholder: "Search Panel board colour..."
                },
    ...
    <script>
        $(document).on('click', '.toggleStatus', function() {
            ...
            $.post("{{ route('panel-board-colour.status') }}", {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(res) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                var url = "{{ route('panel-board-colour.delete', ':id') }}";
        ...
        $('#deleteSelected').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('panel-board-colour.bulkDelete') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
